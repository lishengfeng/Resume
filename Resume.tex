...
{
  \begin{itemize}
    \item \textbf{Labor Hour Management System}: Assisted in generating SQL queries to produce monthly reports for distinct active employees, ensuring accurate workforce tracking and operational transparency.
    \item \textbf{Healthcare Platform (Medicare Project)}: Conducted billing analysis to identify discrepancies, contributing to accurate financial reconciliation and operational efficiency.
    \item \textbf{HR Management System}: Designed KPI displays with a focus on hiring rates, enabling HR personnel to track recruitment effectiveness and improve workforce planning.
    \item \textbf{Project \& Contract (P\&C) System}: Developed a mapping strategy to migrate data from an old VB-based database to PostgreSQL, ensuring a smooth transition and data integrity.
  \end{itemize}
}
